...
"""
import neuron
from pyNN.neuron import setup, run, reset, run_until, run_to_steady_state, run_from_steady_state, end, simulator, \
    Population, SpikeSourcePoisson, SpikeSourceArray, Projection, OneToOneConnector, AllToAllConnector, \
    FromFileConnector, FixedNumberPreConnector, StaticSynapse, NativeCellType, SpikeSourcePoisson, SpikeSourceArray, \
    NoisyCurrentSource, StepCurrentSource
from pyNN.random import RandomDistribution, NumpyRNG
from pyNN import space
from Cortical_Basal_Ganglia_Cell_Classes import Cortical_Neuron_Type, Interneuron_Type, STN_Neuron_Type, GP_Neuron_Type, \
...
if __name__ == '__main__':
    ...
    striatal_spike_times = generate_poisson_spike_times(Pop_size, steady_state_duration, simulation_runtime, 20, 1.0,
                                                        3695)
    ...
    Cortical_Pop_Membrane_Noise = [
        NoisyCurrentSource(mean=0, stdev=0.005, start=steady_state_duration, stop=simulation_duration, dt=1.0) for count
        in range(Pop_size)]
    ...
    controller_call_times = np.arange(steady_state_duration + controller_window_length + controller_sampling_time,
                                      simulation_duration, controller_sampling_time)
    ...
    for controller_call_index, controller_call_time in enumerate(controller_call_times):
        ...
        if controller_call_index + 1 < len(controller_call_times):
            ...
            if next_DBS_pulse_time < controller_call_times[controller_call_index + 1]:
                ...
                updated_DBS_signal[
                window_start_index:window_start_index + new_window_sample_length] = new_DBS_Signal_Segment
                ...
                for i in np.arange(0, num_GPe_Neurons_entrained):
                    ...
                    window_start_index:window_start_index + new_window_sample_length] = GPe_DBS_Signal_Segment
    ...
    DBS_signal = neo.AnalogSignal(DBS_Signal_neuron, units='mA', t_start=0 * pq.ms,
                                  sampling_rate=pq.Quantity(1.0 / simulator.state.dt, '1/ms'))
    DBS_Signal_seg.analogsignals.append(DBS_signal)
    DBS_times = neo.AnalogSignal(DBS_times_neuron, units='ms', t_start=DBS_times_neuron * pq.ms,
                                 sampling_rate=pq.Quantity(1.0 / simulator.state.dt, '1/ms'))

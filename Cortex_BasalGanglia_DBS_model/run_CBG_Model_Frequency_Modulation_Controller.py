...
import neuron
from pyNN.neuron import setup, run_until, end, simulator, Population, Projection, FromFileConnector, StaticSynapse, \
    SpikeSourceArray, NoisyCurrentSource, StepCurrentSource
from pyNN.random import RandomDistribution, NumpyRNG
from pyNN import space
from Cortical_Basal_Ganglia_Cell_Classes import Cortical_Neuron_Type, Interneuron_Type, STN_Neuron_Type, \
...
if __name__ == '__main__':
    ...
    for STN_cell in STN_Pop:
        while(((np.abs(STN_cell.position[0]) > 2000) or (np.abs(STN_cell.position[1]) > 2000)) or
              ((np.abs(STN_cell.position[0]) < 500) and (-1500 < STN_cell.position[1] < 2000))):
    ...
    #                                      MaxValue=250.0)
    # PI Controller:
    controller = StandardPIDController(SetPoint=1.0414e-04, Kp=19.3, Ti=0.2, Td=0, Ts=0.02, MinValue=0.0,
                                       MaxValue=250.0)
